...
class DCC(BaseModule):
    ...
    def _convert_forward_test(self):
        ...
        pose predictions.
        """
        x_bins_ = self.x_bins.view(1, 1, -1).detach()
        y_bins_ = self.y_bins.view(1, 1, -1).detach()
        dim_t = self.spe.dim_t.view(1, 1, 1, -1).detach()
...
class RTMOHead(YOLOXPoseHead):
    ...
    def __init__(
        ...
        if dcc_cfg is not None:
            print('chclkejhpisadiyhfgewayiu')
